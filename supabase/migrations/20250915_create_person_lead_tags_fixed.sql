...
-- Create a simplified view to get all tags for a person as an array
...
WITH tag_data AS (
  SELECT
    ...
    lt.color,
    lt.description
  FROM person_lead_tags plt
  JOIN lead_tags lt ON plt.lead_tag_id = lt.id AND lt.is_active = true
)
SELECT
  p.id as person_id,
  COALESCE(
...
FROM people p
LEFT JOIN tag_data td ON p.id = td.person_id
GROUP BY p.id;
...
GRANT SELECT ON person_tags_view TO authenticated;
GRANT SELECT ON person_tags_view TO anon;
