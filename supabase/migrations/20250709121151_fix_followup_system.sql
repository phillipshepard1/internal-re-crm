-- Fix and simplify follow-up system with frequency-based scheduling
-- Migration: 20250709121151_fix_followup_system.sql
...
BEGIN
  ...
  ELSE
    start_date := last_followup_date::DATE;
  END IF;
  ...
  CASE frequency
    WHEN 'twice_week' THEN
      ...
      IF EXTRACT(DOW FROM start_date) = 1 THEN -- Monday
        next_date := start_date + INTERVAL '3 days'; -- Thursday
      ELSIF EXTRACT(DOW FROM start_date) = 4 THEN -- Thursday
        next_date := start_date + INTERVAL '4 days'; -- Next Monday
      ELSE
        -- Find next Monday or Thursday, whichever comes first
        IF EXTRACT(DOW FROM start_date) < 4 THEN
          next_date := start_date + (4 - EXTRACT(DOW FROM start_date))::INTEGER * INTERVAL '1 day';
        ELSE
          next_date := start_date + (8 - EXTRACT(DOW FROM start_date))::INTEGER * INTERVAL '1 day';
        END IF;
      END IF;
    ...
    WHEN 'weekly' THEN
      ...
      next_date := start_date + ((target_day_of_week + 7 - EXTRACT(DOW FROM start_date)) % 7)::INTEGER * INTERVAL '1 day';
